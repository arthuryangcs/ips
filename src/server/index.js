...
app.post('/api/upload-zip', upload.single('zipFile'), (req, res) => {
  ...
  db.run(insertTaskQuery, [userId], function(err) {
    ...
    try {
        ...
        files.forEach(file => {
          ...
          if (stat.isDirectory()) {
            ...
            setTimeout(() => {
              ...
              // 更新任务进度
              db.run(
                'UPDATE tasks SET progress = ?, completed_files = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
                [progress, completedFiles, progress < 100 ? 'processing' : 'completed', taskId]
              );
      ...
      res.json({ message: '压缩包上传成功，开始检测', taskId });
    } catch (error) {
